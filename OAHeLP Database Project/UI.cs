    ...
    public partial class UI : Form
    {
        ...
        private void uxNamesListBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            using (connection = new SqlConnection(connectionString))
            {
                ...
                string query = $"select S.OAHeLPID, N.FirstName, N.MiddleNames, N.LastName,S.Sex, EG.EthnicGroupName from[Subject].[Subject] S inner join[Subject].SubjectName SN on S.SubjectID = SN.SubjectID inner join[Subject].[Name] N on N.NameID = S.SubjectID inner join[Subject].EthnicGroup EG on S.EthnicGroupID = EG.EthnicGroupID where N.FirstName = '{selectedName}'";
